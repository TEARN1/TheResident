...
-- ==========================================================================
-- SECTION 47 — EVERY TABLE'S GRANTS, WRITTEN DOWN
-- ==========================================================================
--
-- WHY THIS FILE EXISTS
--
-- Until 30 October 2026, Supabase gave every new table in `public` default
-- privileges, so a table was reachable over the Data API the moment it was
-- created. 59 of this project's tables relied on exactly that and never said
-- so: the schema files created them and granted nothing.
--
-- From 30 October new tables get no privileges by default. Tables that
-- already exist keep theirs — so nothing live breaks. What breaks is a
-- REBUILD. Restoring from the schema of record after that date would create
-- 59 tables the app cannot read, with RLS passing and every query returning
-- an empty result and no error. The disaster-recovery path, which exists
-- precisely so the database can be recreated from this repository, would
-- have produced a silently empty app. The restore drill passed only because
-- the old defaults granted everything behind its back.
--
-- HOW IT WAS MADE
--
-- Generated by Postgres from information_schema.role_table_grants on the live
-- project (feevvddvrjmfbhffccbf), not transcribed or inferred. It is the
-- ground truth at the time of writing.
--
-- WHAT WAS DELIBERATELY LEFT OUT
--
-- Only SELECT, INSERT, UPDATE and DELETE are recorded — the privileges the
-- Data API can actually exercise. Live, most tables also grant TRUNCATE,
-- TRIGGER and REFERENCES to `anon` and `authenticated`: the footprint of the
-- old default privileges, not a decision anyone made. TRUNCATE in particular
-- is not subject to row level security. PostgREST exposes no TRUNCATE verb,
-- so it is not reachable through the API today, but recording it here would
-- write an accident into the schema of record as though it were intended.
-- A rebuild from this file therefore produces the same working app with
-- less privilege. Revoking the excess on the live database is a separate,
-- deliberate change — see docs/BACKLOG.md.
--
-- The two res_ tables with no anon/authenticated grants at all —
-- res_client_errors and res_maintenance_runs — are locked down on purpose:
-- both are written only through security-definer functions.
--
-- Guarded by src/store/schemaGrants.test.ts: a res_ table created in any
-- schema file without a grant here (or a stated exemption) fails the build.
--
-- Idempotent: GRANT is a no-op when the privilege is already held, so this is
-- safe to run against the live database as well as a fresh one.

-- res_room_vacancy_watches (section 36) is included: that section had
-- never been applied to production at all — the only thing in the schema of
-- record that was missing live, found by checking all 170 functions and 70
-- tables against the database. It was applied when this section was added.

grant select on public.res_alert_responders to anon;
grant select, insert, update on public.res_alert_responders to authenticated;
grant select, insert, update, delete on public.res_alert_responders to service_role;
grant select on public.res_alerts to anon;
grant select, insert, update on public.res_alerts to authenticated;
grant select, insert, update, delete on public.res_alerts to service_role;
grant select, insert, update on public.res_area_broadcast_receipts to authenticated;
grant select, insert, update, delete on public.res_area_broadcast_receipts to service_role;
grant select on public.res_area_broadcasts to authenticated;
grant select, insert, update, delete on public.res_area_broadcasts to service_role;
grant select on public.res_blocks to anon;
grant select, insert, delete on public.res_blocks to authenticated;
grant select, insert, update, delete on public.res_blocks to service_role;
grant select on public.res_care_circle to anon;
grant select, insert, update on public.res_care_circle to authenticated;
grant select, insert, update, delete on public.res_care_circle to service_role;
grant select on public.res_chore_schedule to anon;
grant select, insert, update, delete on public.res_chore_schedule to authenticated;
grant select, insert, update, delete on public.res_chore_schedule to service_role;
grant select, insert, update, delete on public.res_client_errors to service_role;
grant select on public.res_communities to anon;
grant select, insert, update on public.res_communities to authenticated;
grant select, insert, update, delete on public.res_communities to service_role;
grant select on public.res_community_disputes to anon;
grant select, insert, update on public.res_community_disputes to authenticated;
grant select, insert, update, delete on public.res_community_disputes to service_role;
grant select on public.res_community_invites to anon;
grant select on public.res_community_invites to authenticated;
grant select, insert, update, delete on public.res_community_invites to service_role;
grant select on public.res_community_members to anon;
grant select, insert, delete on public.res_community_members to authenticated;
grant select, insert, update, delete on public.res_community_members to service_role;
grant select, insert on public.res_direct_messages to authenticated;
grant select, insert, update, delete on public.res_direct_messages to service_role;
grant select on public.res_gossip_comments to anon;
grant select, insert, delete on public.res_gossip_comments to authenticated;
grant select, insert, update, delete on public.res_gossip_comments to service_role;
grant select on public.res_gossip_post_reactions to anon;
grant select, insert, delete on public.res_gossip_post_reactions to authenticated;
grant select, insert, update, delete on public.res_gossip_post_reactions to service_role;
grant select on public.res_gossip_posts to anon;
grant select, insert, delete on public.res_gossip_posts to authenticated;
grant select, insert, update, delete on public.res_gossip_posts to service_role;
grant select on public.res_group_buy_pledges to anon;
grant select, insert, update on public.res_group_buy_pledges to authenticated;
grant select, insert, update, delete on public.res_group_buy_pledges to service_role;
grant select on public.res_group_buys to anon;
grant select, insert, update on public.res_group_buys to authenticated;
grant select, insert, update, delete on public.res_group_buys to service_role;
grant select on public.res_handyman_services to anon;
grant select, insert, update on public.res_handyman_services to authenticated;
grant select, insert, update, delete on public.res_handyman_services to service_role;
grant select on public.res_home_areas to anon;
grant select, insert, update, delete on public.res_home_areas to authenticated;
grant select, insert, update, delete on public.res_home_areas to service_role;
grant select on public.res_infra_partner_admins to anon;
grant select on public.res_infra_partner_admins to authenticated;
grant select, insert, update, delete on public.res_infra_partner_admins to service_role;
grant select on public.res_infra_providers to anon;
grant select on public.res_infra_providers to authenticated;
grant select, insert, update, delete on public.res_infra_providers to service_role;
grant select on public.res_jurisdictions to authenticated;
grant select, insert, update, delete on public.res_jurisdictions to service_role;
grant select on public.res_kin_verification_links to anon;
grant select on public.res_kin_verification_links to authenticated;
grant select, insert, update, delete on public.res_kin_verification_links to service_role;
grant select on public.res_lift_bookings to anon;
grant select, update on public.res_lift_bookings to authenticated;
grant select, insert, update, delete on public.res_lift_bookings to service_role;
grant select on public.res_lift_clubs to anon;
grant select, insert, update on public.res_lift_clubs to authenticated;
grant select, insert, update, delete on public.res_lift_clubs to service_role;
grant select on public.res_listings to anon;
grant select, insert, update, delete on public.res_listings to authenticated;
grant select, insert, update, delete on public.res_listings to service_role;
grant select on public.res_lost_found to anon;
grant select, insert, update on public.res_lost_found to authenticated;
grant select, insert, update, delete on public.res_lost_found to service_role;
grant select, insert, update, delete on public.res_maintenance_runs to service_role;
grant select on public.res_market_items to anon;
grant select, insert, update, delete on public.res_market_items to authenticated;
grant select, insert, update, delete on public.res_market_items to service_role;
grant select on public.res_moderation_actions to anon;
grant select on public.res_moderation_actions to authenticated;
grant select, insert, update, delete on public.res_moderation_actions to service_role;
grant select on public.res_neighbourhood_status to anon;
grant select, insert on public.res_neighbourhood_status to authenticated;
grant select, insert, update, delete on public.res_neighbourhood_status to service_role;
grant select on public.res_notice_events to anon;
grant select, insert, update on public.res_notice_events to authenticated;
grant select, insert, update, delete on public.res_notice_events to service_role;
grant select on public.res_notification_prefs to anon;
grant select, insert, update, delete on public.res_notification_prefs to authenticated;
grant select, insert, update, delete on public.res_notification_prefs to service_role;
grant select on public.res_org_broadcast_receipts to anon;
grant select, insert, update on public.res_org_broadcast_receipts to authenticated;
grant select, insert, update, delete on public.res_org_broadcast_receipts to service_role;
grant select on public.res_org_broadcasts to anon;
grant select, insert on public.res_org_broadcasts to authenticated;
grant select, insert, update, delete on public.res_org_broadcasts to service_role;
grant select on public.res_org_follows to anon;
grant select, insert, delete on public.res_org_follows to authenticated;
grant select, insert, update, delete on public.res_org_follows to service_role;
grant select on public.res_org_memberships to anon;
grant select, insert, delete on public.res_org_memberships to authenticated;
grant select, insert, update, delete on public.res_org_memberships to service_role;
grant select on public.res_org_unit_audit to authenticated;
grant select, insert, update, delete on public.res_org_unit_audit to service_role;
grant select on public.res_org_unit_billing to authenticated;
grant select, insert, update, delete on public.res_org_unit_billing to service_role;
grant select on public.res_org_units to anon;
grant select on public.res_org_units to authenticated;
grant select, insert, update, delete on public.res_org_units to service_role;
grant select on public.res_platform_admins to authenticated;
grant select, insert, update, delete on public.res_platform_admins to service_role;
grant select on public.res_profiles to anon;
grant select, insert, update on public.res_profiles to authenticated;
grant select, insert, update, delete on public.res_profiles to service_role;
grant select on public.res_properties to anon;
grant select, insert, update on public.res_properties to authenticated;
grant select, insert, update, delete on public.res_properties to service_role;
grant select on public.res_purchases to anon;
grant select on public.res_purchases to authenticated;
grant select, insert, update, delete on public.res_purchases to service_role;
grant select on public.res_rate_limits to anon;
grant select on public.res_rate_limits to authenticated;
grant select, insert, update, delete on public.res_rate_limits to service_role;
grant select on public.res_reports to anon;
grant select, insert on public.res_reports to authenticated;
grant select, insert, update, delete on public.res_reports to service_role;
grant select on public.res_reputation to anon;
grant select on public.res_reputation to authenticated;
grant select, insert, update, delete on public.res_reputation to service_role;
grant select on public.res_reviews to anon;
grant select, update, delete on public.res_reviews to authenticated;
grant select, insert, update, delete on public.res_reviews to service_role;
grant select on public.res_room_occupants to anon;
grant select on public.res_room_occupants to authenticated;
grant select, insert, update, delete on public.res_room_occupants to service_role;
grant select on public.res_room_requests to anon;
grant select, insert, update on public.res_room_requests to authenticated;
grant select, insert, update, delete on public.res_room_requests to service_role;
grant select on public.res_roommate_seekers to anon;
grant select, insert, update on public.res_roommate_seekers to authenticated;
grant select, insert, update, delete on public.res_roommate_seekers to service_role;
grant select on public.res_rooms to anon;
grant select, insert, update, delete on public.res_rooms to authenticated;
grant select, insert, update, delete on public.res_rooms to service_role;
grant select on public.res_saved_pins to anon;
grant select, insert, update, delete on public.res_saved_pins to authenticated;
grant select, insert, update, delete on public.res_saved_pins to service_role;
grant select on public.res_saved_searches to anon;
grant select, insert, update, delete on public.res_saved_searches to authenticated;
grant select, insert, update, delete on public.res_saved_searches to service_role;
grant select, insert on public.res_security_logs to anon;
grant select, insert on public.res_security_logs to authenticated;
grant select, insert, update, delete on public.res_security_logs to service_role;
grant select on public.res_service_dispatches to anon;
grant select, insert on public.res_service_dispatches to authenticated;
grant select, insert, update, delete on public.res_service_dispatches to service_role;
grant select on public.res_service_report_confirmations to anon;
grant select on public.res_service_report_confirmations to authenticated;
grant select, insert, update, delete on public.res_service_report_confirmations to service_role;
grant select on public.res_service_report_updates to anon;
grant select on public.res_service_report_updates to authenticated;
grant select, insert, update, delete on public.res_service_report_updates to service_role;
grant select on public.res_service_reports to anon;
grant select on public.res_service_reports to authenticated;
grant select, insert, update, delete on public.res_service_reports to service_role;
grant select on public.res_shared_resources to anon;
grant select, insert, update on public.res_shared_resources to authenticated;
grant select, insert, update, delete on public.res_shared_resources to service_role;
grant select on public.res_skills to anon;
grant select, insert, update on public.res_skills to authenticated;
grant select, insert, update, delete on public.res_skills to service_role;
grant select on public.res_subscriptions to anon;
grant select on public.res_subscriptions to authenticated;
grant select, insert, update, delete on public.res_subscriptions to service_role;
grant select on public.res_tool_library to anon;
grant select, insert, update on public.res_tool_library to authenticated;
grant select, insert, update, delete on public.res_tool_library to service_role;
grant select on public.res_traffic_reports to anon;
grant select, insert, delete on public.res_traffic_reports to authenticated;
grant select, insert, update, delete on public.res_traffic_reports to service_role;
grant select on public.res_trust_connections to anon;
grant select, insert, update on public.res_trust_connections to authenticated;
grant select, insert, update, delete on public.res_trust_connections to service_role;
grant select on public.res_unit_verification_requests to authenticated;
grant select, insert, update, delete on public.res_unit_verification_requests to service_role;
grant select on public.res_utility_tokens to anon;
grant select, insert, update on public.res_utility_tokens to authenticated;
grant select, insert, update, delete on public.res_utility_tokens to service_role;
grant select on public.res_vendors to anon;
grant select, insert, update on public.res_vendors to authenticated;
