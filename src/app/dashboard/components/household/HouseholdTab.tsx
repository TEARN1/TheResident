  ...
  if (!householdListingId) {
    return (
      <div className="glass-panel p-12 text-center">
         <Home size={48} className="mx-auto mb-4 text-content-subtle" />
         <h3 className="text-xl font-bold text-content mb-2">No Active Household</h3>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           <h2 className="text-2xl font-bold text-content">{householdName}</h2>
           <p className="text-content-muted text-sm">Household Management & Shared Responsibilities</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="space-y-6">
            <h3 className="text-lg font-bold text-content flex items-center gap-2">
               <Users size={20} className="text-accent" /> Housemates
            </h3>
            <div className="glass-panel p-4 space-y-4">
               {members.map(member => (
                 <div key={member.userId} className={`flex items-center justify-between p-3 rounded-xl border ${member.userId === currentUserId ? 'bg-accent/5 border-accent/20' : 'bg-surface-sunken/20 border-subtle'}`}>
                    <div className="flex items-center gap-3">
                       <div className="w-8 h-8 bg-surface-raised rounded-full flex items-center justify-center text-xs font-bold text-accent">
            ...
            <div className="glass-panel p-4 bg-info/5 border-info/10">
            ...
            {activeTenancies.length > 0 && (
              <div className="glass-panel p-4 space-y-3">
                 <p className="text-xs font-bold text-content flex items-center gap-2">
                    <DoorOpen size={16} className="text-accent" /> Tenancy
                 </p>
         ...
         <div className="lg:col-span-2 space-y-6">
            <h3 className="text-lg font-bold text-content flex items-center gap-2">
               <Award size={20} className="text-accent" /> Active Tasks & Chores
            </h3>
            ...
            <div className="glass-panel p-6 space-y-4">
               <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-default pb-3">
                  <h3 className="text-base font-bold text-content flex items-center gap-2">
                     <ShoppingCart size={18} className="text-accent" /> Shared Grocery & Supplies Checklist
                  </h3>
                  <div className="flex items-center gap-3 text-xs">
                     <span className="text-content-muted">Bought: <strong className="text-success">R{totalBoughtCost}</strong></span>
                     <span className="text-content-subtle">·</span>
                     <span className="text-content-muted">Pending: <strong className="text-accent">R{totalUnboughtCost}</strong></span>
                  </div>
               </div>
               ...
               <form onSubmit={handleAddGrocery} className="flex gap-2">
                  <input
                     ...
                     className="flex-1 bg-surface border border-default rounded-xl px-3 py-2 text-xs text-content outline-none focus:border-accent/50"
                  />
                  <input
                     type="number"
                     min={1}
               ...
               <div className="space-y-2">
                  {groceryItems.map(item => (
                     <div key={item.id} className="flex items-center justify-between p-2.5 rounded-xl bg-surface-sunken/40 border border-subtle text-xs">
                        <label className="flex items-center gap-3 cursor-pointer flex-1">
                           ...
                           <span className={item.bought ? 'line-through text-content-muted font-bold' : 'text-content font-medium'}>{item.name}</span>
                        </label>
                        <div className="flex items-center gap-3">
                           <span className="font-mono text-accent font-bold">R{item.estPrice}</span>
                           <button onClick={() => deleteGroceryItem(item.id)} className="text-content-subtle hover:text-danger p-1"><Trash2 size={13} /></button>
                        </div>
                     </div>
                  ))}
               </div>
                     ...
                     <button
                        ...
                        className={`p-3 rounded-xl border text-xs font-bold transition-all text-left flex flex-col gap-1 ${
                           status === 'low'
                              ...
                              : 'bg-surface-sunken/40 border-default text-content hover:border-accent/30'
                        }`}
                     >
               ...
               <div className="glass-panel p-5 space-y-3">
                  ...
                  <form onSubmit={handleAddGuestLog} className="flex gap-2">
                     <input
                        ...
                        className="flex-1 bg-surface border border-default rounded-lg px-2.5 py-1.5 text-xs text-content outline-none"
                     />
                     <input
                  ...
                  <div className="space-y-1.5">
                     {guestLogs.map(g => (
                        <div key={g.id} className="flex items-center justify-between text-xs p-2 bg-surface-sunken/40 rounded-lg border border-subtle">
                           <span className="text-content font-medium">{g.guestName}</span>
                           <span className="text-accent font-bold">{g.nights} night(s)</span>
                        </div>
                     ))}
                  </div>
               </div>
                  ...
                  <button
                     ...
                     className={`w-full p-3 rounded-xl border text-xs font-bold transition-all flex items-center justify-between ${
                        quietHoursActive
                           ...
                           : 'bg-surface-sunken/40 border-default text-content-muted'
                     }`}
                  >
