...
Twenty-eight things, worst first. Every number here was measured this session,
...
**1. Production has not deployed for thirteen commits.** Every Vercel
production build on `main` since `020b1dd` failed, and every GitHub Actions
run on both workflows failed with it. The live site is serving an older build;
none of the redesign or the eleven new features have reached a single user.

Verified first-hand by building `main` from a clean checkout: it fails with
`module-not-found` on eight files. The causes are mechanical — nine imports at
the wrong depth (`tactileSounds.ts` and `gruvsEvents.ts` are in `src/utils/`,
so a file in `src/app/dashboard/components/<area>/` must climb four levels;
eight climbed three and one climbed two, while two files in the same folders
had it right), plus `dashboard/layout.tsx` using `motion` and `AnimatePresence`
in five places without importing either. The lint job failed separately on four
`any` and two setState-in-effect.

Fixed on **`claude/fix-main-ci`**, pushed, not merged. Both workflows and the
production build verified green end to end on that branch. Nothing else is in
that diff, so it can be reviewed as exactly what it is.

**2. Vercel deployment storage is at 100% of the free tier (10 GB)**, warned
...
**5. Main still carries both Preflight bugs.** It has no `border-style` rule
...
**6. The design system is largely a fiction.** 311 raw `<button>` in app code;
...
**7. Most forms are unlabelled for a screen reader.** 112 `<label>` elements;
...
**8. 26 of 167 icon-bearing buttons have no accessible name** — 13 are the
...
**9. Three tab strips are not tabs** — no `role`, no `aria-selected`. One is
Create Profile / Log In, the first screen anyone meets. `Tabs.tsx` does it
correctly; one file uses it.

**10. Everyday notifications never reach a phone.** `web-push-send` is deployed
...
**11. `push-notify` is still ACTIVE in production** — the stale Expo function
...
**12. No test ever renders a component. Zero.** Every test is static analysis or
...
**13. Everything fixed this session is unverified where it matters.** Borders,
...
**14. Leaked-password protection is disabled.** One toggle in the Supabase
dashboard.

**15. `leaderboard_snapshot` is a materialized view exposed over the Data API
with RLS off** — the one ERROR-level advisory on the project.

**16. 33 tables have RLS enabled and zero policies**, which in Postgres means
...
**17. PostGIS is installed in the `public` schema, and `is_admin()` is
executable by `anon`.** Both Gruvs-side; both worth raising with them.

**18. PITR and the backup cron are unverified.** The restore drill passed
...
**19. "My Business" is effectively unreachable** — one link buried on the
profile page, and the nav entry only appears once you are already on it. That
is the monetisation surface with no route in.

**20. `LiveLocationToggle` says "coming soon"** in the shipped app.

**21. The Gossip feed.** Called too basic, with a lot missing. Untouched,
because what it should become has never been written down.

**22. Four features are deferred with no plan**: next-of-kin verification
links, property deletion with safe cascading, reporting a *person* rather than
a post, and follow.

**23. The Automation Hub** is dev-gated but still compiled into the production
bundle.
...
**24. `src/store/index.ts` is 3,052 lines.** `VibeMap.tsx` is 1,707,
`housing/page.tsx` 1,295. These are where bugs hide.

**25. The map is probably unusable non-visually.** The honest fix may be a text
alternative — "3 reports within 2km: …" — rather than making a canvas
navigable.

**26. 2.3MB of client JS**, largest chunk 241K, for an app aimed at South
African phones on mobile data.

**27. The smoke test proves almost nothing about the real app.** 10 of its 15
...
**1** — merge `claude/fix-main-ci`. Thirteen commits of work are sitting in a
repository and reaching nobody. It is already written and already verified;
the only thing between it and a working deployment is a review.

**12** — render tests. It is the item that would have caught 7, 8 and the
border bug before anyone saw them, and it is the reason to trust or distrust
everything else recorded here.

**6** — adopting the components collapses 7, 8 and 9 into migration work
instead of three separate campaigns.

1 is unblocked and should go today. 6 and 12 both wait on **4**, the fork,
which is the decision everything else is queued behind.
